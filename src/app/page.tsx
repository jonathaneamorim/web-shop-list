        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          <div className="order-2 md:order-1 relative p-8 bg-white rounded-[3rem] border border-gray-100 shadow-xl group animate-in fade-in slide-in-from-left-10 duration-700 hover:shadow-2xl transition-all">
            ...
            <div className="flex justify-center items-center gap-4 mb-10 mt-4">
              <div className="w-16 h-16 rounded-2xl bg-blue-600 flex items-center justify-center text-white font-black text-xs uppercase tracking-widest shadow-lg z-10">Você</div>
              <div className="flex-1 h-2 bg-gray-50 rounded-full relative overflow-hidden flex items-center">
                <div className="w-8 h-8 bg-green-500 rounded-xl absolute left-0 animate-[ping_2s_cubic-bezier(0,0,0.2,1)_infinite] opacity-75" />
                <div className="w-8 h-8 bg-green-500 rounded-xl absolute left-0 animate-[slide_2s_ease-in-out_infinite] flex items-center justify-center text-[8px] text-white font-black">CSV</div>
                <style jsx>{`
                  @keyframes slide {
              ...
              <div className="w-16 h-16 rounded-2xl bg-purple-500 flex items-center justify-center text-white font-black text-xs uppercase tracking-widest shadow-lg z-10">Amigo</div>
            </div>
            
            <div className="bg-gray-50 p-8 rounded-[2rem] border border-gray-100 space-y-4 text-center">
              <div className="inline-flex items-center justify-center w-12 h-12 bg-white rounded-full shadow-sm text-green-500 mb-2">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
              </div>
              <p className="text-gray-400 font-bold uppercase text-[10px] tracking-widest">Arquivo Exportado</p>
              <div className="h-2 w-1/2 bg-gray-200 rounded-full mx-auto" />
              <div className="h-2 w-1/3 bg-gray-200 rounded-full mx-auto" />
            </div>
          </div>
          ...
          <div className="order-1 md:order-2 space-y-8 animate-in fade-in slide-in-from-right-10 duration-700 text-center md:text-left">
            <span className="inline-block bg-green-50 text-green-600 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest">Compartilhamento Offline</span>
            <h2 className="text-4xl md:text-6xl font-black tracking-tighter leading-tight uppercase">
              Envie sua lista, <span className="text-green-500">sem depender de nuvem.</span>
            </h2>
            <p className="text-gray-500 text-lg leading-relaxed">
              Compartilhe suas compras com qualquer pessoa. Basta exportar o arquivo de backup e enviar pelo seu app de mensagens favorito. Quem receber só precisa importar.
            </p>
            <ul className="space-y-4 inline-block text-left">
              {["Exportação simples para arquivo CSV", "Envie pelo WhatsApp, Telegram ou Email", "Importação fácil com um toque"].map((item, i) => (
                <li key={i} className="flex items-center gap-3 font-bold text-gray-700 italic uppercase text-xs md:text-sm">
      ...
      <section className="min-h-screen w-full flex items-center bg-blue-600 text-white overflow-hidden relative py-20">
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-16 items-center relative z-10">
          ...
          <div className="order-1 md:order-2 space-y-8 text-center md:text-left animate-in fade-in slide-in-from-right-10 duration-1000">
            <span className="inline-block bg-white/20 text-white px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest border border-white/10">Privacidade Absoluta</span>
            <h2 className="text-4xl md:text-6xl font-black tracking-tighter leading-tight uppercase">
              Seus dados <span className="text-blue-200">ficam no seu celular.</span>
            </h2>
            <p className="text-blue-100 text-lg leading-relaxed max-w-lg mx-auto md:mx-0">
              Não exigimos login, não guardamos nada em servidores e não rastreamos suas compras. Seus dados são salvos apenas no seu dispositivo.
            </p>
      ...
      <footer className="bg-black text-white py-20 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-10">
          ...
          <div className="text-center md:text-right">
            ...
            <p className="text-gray-600 mt-2 font-medium text-xs">Design focado em privacidade, performance e simplicidade.</p>
          </div>
        </div>
      </footer>
    </div>
